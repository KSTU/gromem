...
!					&SubAtomVX(i,j,k),SubAtomVY(i,j,k),SubAtomVZ(i,j,k)
		...
		do i=1,MemAtom
			...
			print *, MemMol(i), MemName(i), MemX(i), MemY(i), MemZ(i)
   MemMol(i)=MemMol(i)-TempInt
		...
		if(MemType .eq. 5) then
			...
			do i=1,WallAtom
				read(8,'(i5,2a5,i5,3f8.3,3f8.4)') TempInt,TempString,TempString1,&
				&TempInt1,WallX(i),WallY(i),WallZ(i),&
				&TempR1,TempR2,TempR3
...
!		enddo
	...
	if((MemType .eq. 1) .or. (MemType .eq. 2) .or. (MemType .eq. 3) .or. (MemType .eq. 4)) then
		write(31,'(i6)') Ntot+MemAtom
	endif
	if (MemType .eq. 5) then
		write(31,'(i6)') Ntot+MemAtom+WallAtom
	endif
	TempInt=1
	TempInt1=1
	do i=1,SubNum
...
!	enddo
	...
	if(MemType .eq. 5) then
		do i=1,WallAtom
			write(31,'(i5,2a5,i5,3f8.3,3f8.4)') MemMol(MemAtom)+TempInt1,'WAL', 'W',&
			&TempInt,WallX(i),WallY(i),WallZ(i),&
			&0.0,0.0,0.0
			TempInt=TempInt+1
			TempInt1=TempInt1+1
		enddo
...
!		endif
  ...
  write(9,'(i6   i6   i6)')  PolyMol, FixAtom, MemTotAtom
	...
	elseif(MemType .eq. 5) then
		write(42,'(a10,i6)') 'MEM', MemAtom
		write(42,'(a10,i6)') 'WAL', WallAtom
	else
